...
class PmcSpider(scrapy.Spider):
    ...
    def term_requests(self):
        ...
        yield scrapy.Request(url=url,callback=self.term_parse,headers=self.headers)
    ...
    def changePage(self):
        ...
        # 根据本地文件改变表单数据
        with open('settings/formSettings.json','r',encoding='utf-8') as f:
    ...
    def term_parse(self,response):
        ...
        yield FormDataItem(lastQueryKey=lastQueryKey,currentPage='1')

        a_list = response.xpath('//div[@class="rslt"]/div/a')
        for a in a_list:
            title = ''.join(a.xpath('.//text()').extract())
            print(title)
        print("------------------")
        print("------------------")
    ...
    def parse(self, response):
        ...
        print("------------------")
        print("------------------")
        with open('PMC.html','w',encoding='utf-8') as f:
